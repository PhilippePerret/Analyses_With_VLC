...
def build
  ...
  @noeudsRel = {}
  puts "noeuds = #{noeuds.inspect}"
  noeuds.each do |node_type, dnode|
    puts "dnode: #{dnode.inspect}"
    @noeudsRel.merge!(node_type => PFANoeudRel.new(self, dnode.merge!(type: node_type)))
  end

  puts "@noeudsRel[:dv] = #{@noeudsRel[:dv].inspect}"
  # puts "\n@noeudsRel: #{@noeudsRel.inspect}"
  prepare_noeuds_abs
  ...
  [:dv, :d2, :dn].each do |kpart|
    puts "noeudRel(#{kpart.inspect}) = #{noeudRel(kpart).inspect}"
  ...
  # cmd = "/usr/local/bin/convert #{image_path} -rotate -90 #{image_path}"
  # res = `#{cmd} 2>&1`
  ...
  begin
    sleep 0.3
  end until File.exist?(image_path)


  # ==== POST PRODUCTION ===
  
  if File.exist?(@image_path)
    puts "Le PFA a été construit avec succès.".vert
  else
