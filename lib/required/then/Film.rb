...
class << self
	...
	# 
	# Le film courants, dans VLC
	# Il doit y en avoir un, ouvert en ce moment ou défini et existant
	# dans les données chaudes de l'application.
	# 
	def current
		@current ||= begin

			film_path = vlc('return path of current item')

			if film_path.empty?

				# 
				# Puisqu'il n'y a pas de film ouvert, on essaie de charger
				# le dernier film étudié, s'il existe encore
				# 
				film_path = try_to_load_last_film

			elsif film_path && film_path != AWV.hotdata[:last_film_path]

				# 
				# Actualisation du film courant s'il est différent du
				# film actuel
				# 
				AWV.update_hotdata(last_film_path: film_path)

			end

			# 
			# Si aucun film n'a pu être déterminé, on doit s'arrêter là
			# 
			if film_path.nil? || film_path.empty?
				puts "[ERREUR FATALE] Il faut ouvrir un film dans VLC avant d'appeler cette commande !".rouge
				exit 1
			end

			# 
			# Sinon, on définit l'instance du film courant
			# 
			new(film_path) 
	...
	# 
	# {String} Chemin d'accès au dernier film étudié
	# 
	# Essai de charger le dernier film étudié
	# 
	def try_to_load_last_film
		film_path = AWV.hotdata[:last_film_path]
		return if film_path.nil?
		if not File.exist?(film_path)
			AWV.update_hotdata(last_film_path: nil)
			return
		end
		# Un film est défini est existe
		# => On le charge
		VLC.open_film(film_path)

		return film_path
	end

	# 
	# Pour tout ré-initialiser concernant le film (sauf les personnages)
	# 
	def reset
		@current = nil
	end
...
# 
# Chemin d'accès à la vidéo du film
# 
attr_reader :path
...
def main_folder
	@main_folder ||= File.join(film_folder, affixe)
end
def film_folder
	@film_folder ||= File.dirname(path)
end
def affixe
